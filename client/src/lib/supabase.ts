...
export function toLicensePayload(form: LicenseRecordForm) {
  return { registration_number: form.registrationNumber.trim() || null, registration_date: form.registrationDate, name: form.name.trim(), birth_date: form.birth || null, phone: form.phone || null, id_card: form.idCard || null, exam_date: form.exam, category: form.category || "B", result: form.result, second_exam_date: form.secondExamDate || null, second_result: form.secondResult || null, vehicle_number: form.vehicleNumber || null, total_amount: form.totalAmount, first_payment: form.firstPayment, second_payment: form.secondPayment, remaining_amount: Math.max(0, form.totalAmount - form.firstPayment - form.secondPayment), notes: form.notes || null, photo_url: form.photoUrl || null, photo_key: form.photoKey || null, updated_at: new Date().toISOString() };
}
