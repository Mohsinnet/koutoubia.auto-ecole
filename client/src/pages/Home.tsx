...
import { Archive, BarChart3, CalendarDays, Camera, Check, ChevronLeft, ChevronRight, ClipboardList, FileSpreadsheet, Images, Menu, Plus, RefreshCw, Search, UserRound, Users, X } from "lucide-react";
...
export default function Home() {
  ...
  async function syncRecords(showToast = false) {
    ...
    else { setConnectionError(null); const rows = (data ?? []) as LicenseRecordRow[]; setRecords(rows.map(toLicenseRecord)); if (showToast) toast.success(`تمت المزامنة — ${rows.length} سجل`); }
  ...
  async function handlePhotoChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    else { const publicUrl = supabase.storage.from("candidate-photos").getPublicUrl(path).data.publicUrl; setForm((current) => ({ ...current, photoUrl: publicUrl, photoKey: path })); toast.success("تم رفع صورة المترشح"); }
