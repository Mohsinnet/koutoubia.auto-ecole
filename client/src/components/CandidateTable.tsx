...
function DesktopTable({ records, removePhoto, setViewingRecord, openEdit, removeRecord }: Omit<CandidateTableProps, "loading" | "openNew">) {
  return <table className="w-full min-w-[1080px] text-right" aria-label="جدول المترشحين"><thead><tr className="sticky top-0 z-10 border-b border-[#e8e1d7] bg-[#f8f6f1] text-[11px] font-bold text-[#6f8584]"><th className="w-[28%] px-5 py-4">المترشح والملف</th><th className="w-[15%] px-4 py-4">العربة / الفئة</th><th className="w-[13%] px-4 py-4">التسجيل</th><th className="w-[20%] px-4 py-4">مواعيد الامتحان</th><th className="w-[16%] px-4 py-4">المدفوعات</th><th className="w-[8%] px-5 py-4 text-center">إجراء</th></tr></thead><tbody className="divide-y divide-[#f0ece6]">{records.map((item) => <DesktopRow key={item.id} item={item} removePhoto={removePhoto} setViewingRecord={setViewingRecord} openEdit={openEdit} removeRecord={removeRecord} />)}</tbody></table>;
}
...
function DesktopRow({ item, removePhoto, setViewingRecord, openEdit, removeRecord }: { item: CandidateRecord; removePhoto: CandidateTableProps["removePhoto"]; setViewingRecord: CandidateTableProps["setViewingRecord"]; openEdit: CandidateTableProps["openEdit"]; removeRecord: CandidateTableProps["removeRecord"] }) {
  return <tr className="group align-middle transition odd:bg-[#fffdf9] hover:bg-[#fff7ed]"><td className="px-5 py-4"><CandidateIdentity item={item} removePhoto={removePhoto} compact /></td><td className="px-4 py-4"><div className="font-semibold text-[#36585d]">{item.vehicleNumber || "بدون عربة"}</div><span className="category-chip mt-2 inline-flex">فئة {item.category}</span><div className="mt-2 text-[11px] text-[#8b9998]">{item.phone || "هاتف غير مسجل"}</div></td><td className="px-4 py-4"><p className="text-sm font-semibold text-[#536d70]">{formatDisplayDate(item.registrationDate)}</p><p className="mt-1 text-[11px] text-[#8b9998]">تاريخ التسجيل</p></td><td className="px-4 py-4"><ExamLine label="الأول" date={item.exam} result={item.result} /><ExamLine label="الثاني" date={item.secondExamDate} result={item.secondResult} /></td><td className="px-4 py-4 text-xs text-[#536d70]"><div className="flex justify-between gap-3"><span>الإجمالي</span><strong>{formatMoney(item.totalAmount)}</strong></div><div className="mt-1 flex justify-between gap-3"><span>المدفوع</span><strong>{formatMoney(item.firstPayment + item.secondPayment)}</strong></div><div className="mt-1 flex justify-between gap-3 font-bold text-[#e8793a]"><span>الباقي</span><strong>{formatMoney(item.remainingAmount)}</strong></div></td><td className="px-5 py-4"><ActionButtons item={item} setViewingRecord={setViewingRecord} openEdit={openEdit} removeRecord={removeRecord} /></td></tr>;
}
...
function CandidateIdentity({ item, removePhoto, compact = false }: { item: CandidateRecord; removePhoto: CandidateTableProps["removePhoto"]; compact?: boolean }) {
  return <div className="flex min-w-0 items-start gap-3"><div className={`${compact ? "h-12 w-12 rounded-xl" : "h-14 w-14 rounded-2xl"} flex shrink-0 items-center justify-center overflow-hidden bg-[#e9f0ed] text-lg font-bold text-[#5f7c7c]`}>{item.photoUrl ? <div className="relative h-full w-full"><img src={item.photoUrl} alt={`صورة ${item.name}`} className="h-full w-full object-cover" /><button type="button" onClick={() => void removePhoto(item)} className="absolute inset-x-1 bottom-1 rounded-lg bg-[#09252b]/75 px-1 py-1 text-[9px] font-bold text-white" title="إزالة الصورة"><ImageOff size={11} className="mx-auto" /></button></div> : item.name.trim().slice(0, 1) || "؟"}</div><div className="min-w-0"><p className="truncate text-sm font-bold text-[#254950]">{item.name}</p><p className="mt-1 truncate text-[11px] text-[#8b9998]">رقم التسجيل: {item.registrationNumber || `#${item.id}`}</p><p className="mt-1 truncate text-[11px] text-[#a2acab]">بطاقة: {item.idCard ? `${item.idCard.slice(0, 5)}••••••` : "غير مسجلة"}</p></div></div>;
}

function ExamLine({ label, date, result }: { label: string; date: string; result: string }) {
  return <div className="flex items-center justify-between gap-2 text-xs"><span className="font-semibold text-[#536d70]">{label}</span><span className="text-[#6f8584]">{date ? formatDisplayDate(date) : "غير مبرمج"}</span>{date && <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${result === "ناجح" ? "bg-[#e9f0ed] text-[#276653]" : result === "راسب" ? "bg-[#fbe9e6] text-[#a64e46]" : "bg-[#fff1e8] text-[#b65d2f]"}`}>{result || "غير محدد"}</span>}</div>;
}
