...
function ExamLine({ label, date, result }: { label: string; date: string; result: string }) {
  return <div className="flex w-full items-center justify-between gap-2 rounded-lg bg-[#faf7f2] px-2.5 py-1.5 text-xs"><span className="shrink-0 font-bold text-[#536d70]">{label}</span><span className="min-w-0 truncate text-[#6f8584]">{date ? formatDisplayDate(date) : "غير مبرمج"}</span>{date && <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${result === "ناجح" ? "bg-[#e9f0ed] text-[#276653]" : result === "راسب" ? "bg-[#fbe9e6] text-[#a64e46]" : "bg-[#fff1e8] text-[#b65d2f]"}`}>{result || "غير محدد"}</span>}</div>;
}
